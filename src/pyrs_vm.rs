...
pub struct PyVM {
    ...
    class_defs: HashMap<String, UserClass>,
...
impl PyVM {
    ...
    fn load_fast(&mut self, i: usize) {
        let obj = std::mem::replace(&mut self.cache_vec[i], Obj::None.into());
        self.push(obj);
    }
    ...
    fn return_value(&mut self) {
        let mut fn_objs = vec![Obj::None.into()];
        ...
        let ret_ptr = fn_objs.pop().unwrap_or(Obj::None.to_arc());
        if ret_ptr.as_ref() == &Obj::None {
            self.push_err(PyException {
                error: PyError::StackError,
